...
void test_parse_basic_invite_request(void) {
  ...
  TEST_ASSERT_EQUAL_STRING_LEN(
      " SIP/2.0/UDP pc33.example.com;branch=z9hG4bK776asdhds", h->value.buf,
      h->value.len);

  h = STAILQ_NEXT(h, _next);
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("To", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" Bob <sip:bob@example.com>", h->value.buf,
                               h->value.len);

  h = STAILQ_NEXT(h, _next);
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("From", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" Alice <sip:alice@example.com>;tag=1928301774",
...
void test_parse_register_request(void) {
  ...
  TEST_ASSERT_EQUAL_STRING_LEN("Via", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(
      " SIP/2.0/UDP host.example.com;branch=z9hG4bK776asdhds", h->value.buf,
      h->value.len);

  h = STAILQ_NEXT(h, _next);
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("To", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" <sip:alice@example.com>", h->value.buf,
                               h->value.len);

  h = STAILQ_NEXT(h, _next);
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("From", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" <sip:alice@example.com>;tag=12345",
                               h->value.buf, h->value.len);

  h = STAILQ_NEXT(h, _next);
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("Call-ID", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" reg1234", h->value.buf, h->value.len);

  h = STAILQ_NEXT(h, _next);
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("CSeq", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" 1 REGISTER", h->value.buf, h->value.len);
...
void test_parse_200_ok_response(void) {
  ...
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("To", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" Bob <sip:bob@example.com>;tag=2493k59kd",
                               h->value.buf, h->value.len);
...
void test_parse_486_busy_here(void) {
  ...
  TEST_ASSERT_EQUAL_STRING_LEN("Via", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(
      " SIP/2.0/UDP server10.example.com;branch=z9hG4bKnashds8", h->value.buf,
      h->value.len);

  h = STAILQ_NEXT(h, _next);
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("To", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" Bob <sip:bob@example.com>;tag=a6c85cf",
                               h->value.buf, h->value.len);
...
void test_parse_100_no_reason_phrase(void) {
  ...
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("Via", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" SIP/2.0/UDP test.com", h->value.buf,
                               h->value.len);

  h = STAILQ_NEXT(h, _next);
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("To", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" <sip:foo@test.com>", h->value.buf,
                               h->value.len);

  h = STAILQ_NEXT(h, _next);
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("From", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" <sip:bar@test.com>;tag=aaa", h->value.buf,
                               h->value.len);

  h = STAILQ_NEXT(h, _next);
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("Call-ID", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" testid", h->value.buf, h->value.len);

  h = STAILQ_NEXT(h, _next);
  TEST_ASSERT_NOT_NULL(h);
  TEST_ASSERT_EQUAL_STRING_LEN("CSeq", h->key.buf, h->key.len);
  TEST_ASSERT_EQUAL_STRING_LEN(" 1 TRYING", h->value.buf, h->value.len);
