...
#ifndef C_MINILIB_SIP_CODEC_PARSE_SIP_PROTO_VER_H
#define C_MINILIB_SIP_CODEC_PARSE_SIP_PROTO_VER_H
...
cmsc_parser_parse_sip_proto_ver(const uint32_t buffer_len, const char *buffer,
  ...
  sip_proto_ver->major = atoi(mini_buf);
  ...
  mini_buf[0] = *(++dot);
  mini_buf[1] = 0;
  sip_proto_ver->minor = atoi(mini_buf);
